...
function Home(){
    ...
    const produkCollection = firestore.collection(`toko/${user.uid}/produk`);
    const transaksiColl = firestore.collection(`toko/${user.uid}/transaksi`);

    const [snapshotProduk, loadingProduk] = useCollection(produkCollection);
    const [snapshotTransaksi, loadingTransaksi] = useCollection(transaksiColl.where('tanggal','==',todayDateString));
    ...
    useEffect(() => {
        if(snapshotProduk){
            setProdukItems(snapshotProduk.docs.filter((produkDoc) => {
                if(filterProduk){
                    return produkDoc.data().nama.toLowerCase().includes(filterProduk.toLocaleLowerCase());
                }
    ...
    return <>
        ...
        <Grid container spacing={5}>
                            ...
                            Object.keys(transaksi.items).map(k=>{
                                ...
                                return (
                                    <TableRow key={k}>
                                        ...
                                        <TableCell><TextField className={classes.inputJumlah} value={item.jumlah} type="number" onChange={handleChangeJumlah(k)} disabled={isSubmitting}/></TableCell>
            ...
            <Grid item xs={12} md={4}>
                <List className={classes.produkList} component="nav" subheader={
                ...
                }>
                    {
                        produkItems.map((produkDoc) => {
                            const produkData = produkDoc.data();
                            return <ListItem key={produkDoc.id} button disabled={!produkData.stok || isSubmitting} onClick={addItem(produkDoc)}>
                                    {
                                        produkData.foto ?
                                        <ListItemAvatar>
